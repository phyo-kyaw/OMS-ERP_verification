...
def initializeNS(Dummy):
	...
	for tryLoop in range(20):
		try:	
			SearchResult = sessionNS.client.factory.create( 'ns4:SearchResult' )
		except WebFault, webErr:
			print webErr
			continue
		break
	else:
		print 'SearchResult creation failed 20 retries!!!!!'
...
def searchRecordsOnNS( item, chunk):
	...
	for tryLoop in range(5):
		try:
			logoutNS(True)
			loginNS()		
			resultTT = sessionNS.searchRecords(singleTSB)
			SearchResult = resultTT[1]
		except WebFault, webErr:
			print webErr
			continue
		break
	...
	if SearchResult.status._isSuccess is True:
		i = 0
		writeSemaphore.acquire()
		f = open(current_process().name + "x" + fileNS[item],"ab")
		c = csv.writer(f)			
		while i <  SearchResult.totalRecords :
				row = SearchResult.recordList.record[i]
		...
		print 'FAILED RESPONSE', SearchResult
		...
		if keyIn <> u'\x1b':
			...
			if runNS :
				...
				if runDiff and runNS :
					...
					'''
					if loopChoices.index(item) == len(loopChoices) - 1 :
						task_queue.put((getDifferenceInP, (item, .1)))
					else:
						task_queue.put((getDifferenceInP, (item, 5)))
					'''
